...
drive.get(
    r'https://www.amazon.com/Best-Sellers-Home-Kitchen-Posters-Prints/zgbs/home-garden/381142011/ref=zg_bs_pg_1_home-garden?_encoding=UTF8&pg=1')  # 跳转到亚马逊
x = 1  # 定义一个计数器给图片命名并计数
for i in range(1, 3):
    drive.wait(2)
    ...
    path = r'C:\Users\Administrator\Desktop\Amz_Best'  # 自定义路径,存放这个排行榜的图片
    time_path = os.path.join(path, day_date)
    if not os.path.exists(time_path):
        os.makedirs(time_path)
    for url in tqdm(image_urls, desc=f'正在爬取Best Sellers in Posters & Prints第{i}页'):
